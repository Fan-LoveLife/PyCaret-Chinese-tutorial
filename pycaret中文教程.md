...
​		PyCaret 的回归模块是一个有监督的机器学习，用于估计因变量与自变量(单个或多个)之间的关系。回归分析是用于预测连续值，比如谁：预测销售额、数量、温度等等；该模块提供了几个[预处理功能](https://pycaret.gitbook.io/docs/get-started/preprocessing) , 通过`setup()` 函数设置为建模准备数据。它有超过 25 种即用型算法和几个图表来分析训练模型的性能。
